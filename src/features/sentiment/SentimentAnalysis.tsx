...
export const SentimentAnalysis: React.FC = () => {
  const [items, setItems] = useState<AnalyzedItem[]>(SEED_ITEMS);
  ...
  const analyze = async () => {
    ...
    const { sentiment, score } = detectSentiment(draft);
    ...
    showToast(`Sentiment: ${SENTIMENT_META[sentiment].label}`, {
      variant:
        sentiment === "positive"
          ? "success"
          : sentiment === "negative"
            ? "error"
            : "info",
      description: `Confidence ${Math.abs(Math.round(score * 100))}%`,
    });
  };
