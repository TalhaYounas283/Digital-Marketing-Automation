...
import { calendarApi, ScheduledPostDto } from "@/services/calendarApi";
...
const dtoToPost = (dto: ScheduledPostDto): ScheduledPost => ({
  id: dto.id,
  title: dto.title,
  platform: (dto.platform as ScheduledPost["platform"]) ?? "instagram",
...
export const ContentCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    calendarApi
      .list({ month, year })
      .then((list) => setPosts(list.map(dtoToPost)))
      .catch(() => {
        /* surface via empty calendar */
      });
  }, [currentDate]);
