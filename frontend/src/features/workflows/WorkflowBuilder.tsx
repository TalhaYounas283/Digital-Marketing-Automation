...
import { workflowsApi, WorkflowDto } from "@/services/workflowsApi";
import {
  Workflow,
  Plus,
  Play,
...
const dtoToWorkflow = (dto: WorkflowDto): AutomationWorkflow => ({
  id: dto.id,
  name: dto.name,
  tool: dto.tool === "n8n" ? "n8n" : "n8n",
  trigger: dto.trigger,
  action: dto.action,
  status: dto.status,
  lastRun: dto.lastRun ? new Date(dto.lastRun).toLocaleString() : "Never",
});
...
export const WorkflowBuilder: React.FC = () => {
  ...
  useEffect(() => {
    workflowsApi
      .list()
      .then((list) => setWorkflows(list.map(dtoToWorkflow)))
      .catch((err) =>
  ...
  const toggleStatus = async (id: string) => {
    ...
    try {
      ...
      setWorkflows((prev) =>
        prev.map((w) => (w.id === id ? dtoToWorkflow(updated) : w)),
      );
      showToast(
      ...
      showToast("Could not change workflow state", {
        variant: "error",
        description: err instanceof Error ? err.message : "Unknown error",
      });
    }
  ...
  const removeWorkflow = async (id: string) => {
    try {
      ...
      showToast("Could not delete workflow", {
        variant: "error",
        description: err instanceof Error ? err.message : "Unknown error",
      });
    }
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      const newWorkflow = dtoToWorkflow(created);
